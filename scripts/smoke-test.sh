...
#   B. 运行时断言(仅 Linux/WSL/Termux):执行 nasm/nasd、start/status/log/
#      restart/stop、健康端口
...
if [ "$IS_LINUX" = "0" ]; then
  step "运行环境"
  warn_pill() { printf '\033[33m [i]%s\033[0m\n' "$*"; }
  warn_pill "检测到非 Linux($OS):运行时断言(B 层)将跳过,仅验证机制(A 层)。"
  warn_pill "完整测试请在 Termux / WSL2 / CI 的 Linux 下运行。"
...
go build -trimpath -ldflags "$LDFLAGS" -o "$SMOKE/dist/nasm-android-arm64" ./cmd/nasm || { bad "构建 nasm 失败"; exit 1; }
...
for f in nasm nasd; do
  [ -s "$NAS_ROOT/bin/$f" ] || { bad "缺少非空二进制 $f"; exit 1; }
done
ok "二进制已放置到 bin/(非空)"
if [ "$IS_LINUX" = "1" ]; then
  [ -x "$NAS_ROOT/bin/nasm" ] && [ -x "$NAS_ROOT/bin/nasd" ] || { bad "缺少可执行位"; exit 1; }
  V_NA=$( "$NAS_ROOT/bin/nasm" -version 2>/dev/null || "$NAS_ROOT/bin/nasm" version 2>/dev/null || true )
  ok "nasm 可执行($V_NA)"
else
  echo "  [i] 非 Linux:跳过可执行位/运行探测(将在 Termux/WSL 校验)"
fi

# ---------- 3. 机制: 重装同版本应跳过(严格断言依赖 Linux 的可执行位/版本探测) ----------
...
rm -f "$NAS_ROOT/bin/nasd.bak" "$NAS_ROOT/bin/nasm.bak"
bash "$ROOT/nas.sh" update -f || { bad "update -f 失败"; exit 1; }
[ -f "$NAS_ROOT/bin/nasd.bak" ] && [ -f "$NAS_ROOT/bin/nasm.bak" ] || { bad "-f 后缺少 .bak 备份"; exit 1; }
ok "强制替换后 .bak 备份存在"

# ---------- 5. 机制: SHA256 篡改必须拒绝且无副作用 ----------
step "机制: 校验和篡改防护"
TAMPER="$SMOKE/tamper"; mkdir -p "$TAMPER"
cp "$SMOKE/dist/nasm-android-arm64" "$TAMPER/"
cp "$SMOKE/dist/nasd-android-arm64" "$TAMPER/"
printf '%064d  nasd-android-arm64\n%064d  nasm-android-arm64\n' 0 0 > "$TAMPER/sha256sums.txt"
...
rm -rf "$SMOKE/nas" "$SMOKE/nas2" 2>/dev/null
